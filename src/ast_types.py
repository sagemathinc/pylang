...
class AST_Token(AST):
    ...
    properties = {
        ...
        'col':
        'The column number at which the token occurs',
        'pos':
        '',
        'endpos':
        '',
        ...
        'file':
        'The filename in which this token occurs',
        'leading_whitespace':
            ...
            if Array.isArray(self[key]):
                if self[key].length:
                    ...
                    if depth > 1:
                        ...
                        for element in self[key]:
                            p(pad + '   ' + yellow +
                              element.constructor.name.slice(4) + reset)
                    p(pad + ' ]')
                else:
                    p(pad + ' ' + blue + key + ': ' + reset + '[]')
            elif self[key]:
                if is_node_type(self[key], AST):
                    ...
                    if tname is 'Token':
                        p(pad + ' ' + blue + key + ': ' + magenta + tname +
                          reset)
                        for property in self[key]:
                            p(pad + '   ' + blue + property + ': ' + reset +
                              self[key][property])
                    else:
                        p(pad + ' ' + blue + key + ': ' + yellow + tname +
                          reset)
                        if depth > 1:
                            self[key]._dump(depth - 1, omit, offset + 1, False)
                elif jstype(self[key]) is "string":
                    p(pad + ' ' + blue + key + ': ' + green + '"' + self[key] +
                      '"' + reset)
...
class AST_Block(AST_Statement):
    ...
    properties = {'body': "[AST_Statement*] an array of statements"}
...
class AST_ListComprehension(AST_ForIn):
    ...
    properties = {
        ...
        'statement':
        "[AST_Node] statement to perform on each element before returning it"
    }
...
class AST_DictComprehension(AST_ListComprehension):
    ...
    properties = {
        'value_statement':
        "[AST_Node] statement to perform on each value before returning it",
...
class AST_Toplevel(AST_Scope):
    ...
    properties = {
        ...
        'imports':
        "[Object/S] a map of module_id->AST_Toplevel for all imported modules (this represents all imported modules across all source files)",
        'imported_module_ids':
        "[string*] a list of module ids that were imported by this module, specifically",
        'nonlocalvars':
        "[String*] a list of all non-local variable names (names that come from the global scope)",
        ...
        'classes':
        "[Object/S] a map of class names to AST_Class for classes defined in this module",
        'filename':
        "[string] The absolute path to the file from which this module was read",
...
class AST_Import(AST_Statement):
    ...
    properties = {
        ...
        'key':
        "[string] The key by which this module is stored in the global modules mapping",
        'alias':
...
class AST_Decorator(AST_Node):
    ...
    properties = {'expression': "[AST_Node] the decorator expression"}
...
class AST_Class(AST_Scope):
    ...
    properties = {
        'name':
        "[AST_SymbolDeclaration?] the name of this class",
        'init':
        "[AST_Function] constructor for the class",
        'parent':
...
class AST_Yield(AST_Return):
    ...
    properties = {
        'is_yield_from':
        "[bool] True iff this is a yield from, False otherwise"
    }
...
class AST_Definitions(AST_Statement):
    ...
    properties = {'definitions': "[AST_VarDef*] array of variable definitions"}
...
class AST_BaseCall(AST_Node):
    ...
    properties = {'args': "[AST_Node*] array of arguments"}
...
class AST_ItemAccess(AST_PropAccess):
    ...
    properties = {
        'assignment':
        "[AST_Node or None] Not None if this is an assignment (a[x] = y) rather than a simple access",
    }
...
class AST_Splice(AST_PropAccess):
    ...
    properties = {
        'property2':
        "[AST_Node] the 2nd property to access - typically ending index for the array.",
...
class AST_Binary(AST_Node):
    ...
    def _walk(self, visitor):
        return visitor._visit(
            self,
            lambda: [self.left._walk(visitor),
...
class AST_Existential(AST_Node):
    ...
    properties = {
        ...
        'after':
        "[None|string|AST_Node] is None when there is nothing following this operator, is a string when there is as AST_PropAccess following this operator, is an AST_Node if it is used a a shorthand for the conditional ternary, i.e. a ? b == a if a? else b",
    }
...
class AST_Set(AST_Node):
    "A set literal"
    properties = {'items': "[AST_SetItem*] array of items"}
...
class AST_String(AST_Constant):
    "A string literal"
    properties = {'value': "[string] the contents of this string"}


class AST_Verbatim(AST_Constant):
    "Raw JavaScript code"
    properties = {'value': "[string] A string of raw JS code"}


class AST_Number(AST_Constant):
    "A number literal"
    properties = {'value': "[number] the numeric value"}


class AST_RegExp(AST_Constant):
    "A regexp literal"
    properties = {'value': "[RegExp] the actual regexp"}
    ...
    def in_boolean_context(self):
        ...
        while i > 0:
            ...
            if (is_node_type(p, AST_If) and p.condition is self
                    ...
                    and p.expression is self):
                return True
            if not (is_node_type(p, AST_Binary) and
                    (p.operator is "&&" or p.operator is "||")):
                return False
            self = p
...
def has_calls(expression):
    ...
    try:
        ...
        def is_call(node):
            if is_node_type(node, AST_BaseCall) or is_node_type(
                    node, AST_ItemAccess):
                raise Found()
